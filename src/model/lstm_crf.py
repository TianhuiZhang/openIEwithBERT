...
class BLSTM_CRF(object):
    ...
    def add_blstm_crf_layer(self, crf_only):
        ...
        if self.is_training:
            ...
            self.embedded_chars = tf.keras.layers.Dropout(rate=0.1)(self.embedded_chars)
    ...
    def add_blstm_layer(self):
        ...
        lstm_output = tf.keras.layers.Bidirectional(
               tf.keras.layers.CuDNNLSTM(FLAGS.max_seq_length, return_sequences=True))(self.embedded_chars)
        #LSTM using keras directly
        lstm_output = tf.keras.layers.Bidirectional(
               tf.keras.layers.CLSTM(FLAGS.max_seq_length, return_sequences=True))(self.embedded_chars)
        '''
        #LSTM CELL
        lstm_output = self.blstm_layer(self.embedded_chars)
        return lstm_output
    ...
    def bilstmcell(self):
        cell_forward = rnn.LSTMCell(self.hidden_unit)
        cell_backward = rnn.LSTMCell(self.hidden_unit)
        if self.dropout_rate is not None:
            cell_backward = rnn.DropoutWrapper(cell_backward, output_keep_prob=self.dropout_rate)
            cell_forward = rnn.DropoutWrapper(cell_forward, output_keep_prob=self.dropout_rate)
        return cell_forward, cell_backward

    def blstm_layer(self, embedding_chars):
        with tf.variable_scope('rnn_layer'):
            cell_forward, cell_backward = self.bilstmcell()
            if self.num_layers > 1:
                cell_forward = rnn.MultiRNNCell([self.bilstmcell()[0] for _ in range(self.num_layers)], state_is_tuple=True)
                cell_backward = rnn.MultiRNNCell([self.bilstmcell()[0] for _ in range(self.num_layers)], state_is_tuple=True)
            outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell_forward, cell_backward, embedding_chars,
                                                         dtype=tf.float32)
